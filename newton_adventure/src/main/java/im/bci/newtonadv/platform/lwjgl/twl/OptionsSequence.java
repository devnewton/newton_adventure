...
public class OptionsSequence implements IOptionsSequence {
    ...
    public void update() throws TransitionException {
        if (optionsGui.okPressed) {
            ...
            try {
                ...
                Logger.getLogger(OptionsSequence.class.getName()).log(Level.SEVERE, "cannot save options", ex);
            }
    ...
    private void writeConfig(String path) throws FileNotFoundException, IOException {
        FileOutputStream os = new FileOutputStream(path);
        try {
            config.store(os, "Newton adventure configuration, see " + PlatformFactory.getDefaultConfigFilePath() + " for example and documentation");
        } finally {
            os.close();
        }
    ...
    private void updateConfig() {
        ...
        config.setProperty("view.bpp", "" + Display.getDisplayMode().getBitsPerPixel());
        ...
        config.setProperty("key.rotate_clockwise", getKeyFieldName(input.keyRotateClockwise));
    ...
    private String getKeyFieldName(int key) {
        for (Field field : Keyboard.class.getFields()) {
            try {
                if (field.getName().startsWith("KEY_") && field.getInt(null) == key) {
                    return field.getName();
                }
            } catch (Exception ex) {
                Logger.getLogger(OptionsSequence.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
        Logger.getLogger(OptionsSequence.class.getName()).log(Level.SEVERE, "Unknow lwjgl key value {0}", key);
        return "";
    }
}
