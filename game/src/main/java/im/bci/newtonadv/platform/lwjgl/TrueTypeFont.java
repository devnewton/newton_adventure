...
public class TrueTypeFont implements ITrueTypeFont {
    ...
    private BufferedImage getCharacterImage(int charwidth, int charheight,
            char ch) {

        if (specialCharacters.containsKey(ch)) {
            try {
                InputStream is = gameData.openFile(specialCharacters.get(ch));
                try {
    ...
    private void drawString(float x, float y, String whatchars, float scaleX,
            float scaleY) {
    ...
    private void drawString(float x, float y, String whatchars, int startIndex,
            int endIndex, float scaleX, float scaleY, Align format) {
